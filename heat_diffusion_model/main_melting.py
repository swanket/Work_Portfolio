...
fig,ax = plt.subplots(nrows = 1,ncols = 1,figsize = (10,7))
ax.semilogx(visc[:,np.argmin(np.abs(t/tau_d - 1))],z/1000,color = 'r')
ax.semilogx(visc[:,np.argmin(np.abs(t/tau_d - 3.25))],z/1000,color = 'b')
ax.semilogx(visc[:,np.argmin(np.abs(t/tau_d - 3.4))],z/1000,color = 'k')
ax.invert_yaxis()
ax.set_ylabel('Depth (km)')
